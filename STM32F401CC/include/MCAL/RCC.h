 ...
 *
 *
 * @param CLK_clock: CLK_clock to control
 * @param RCC_STAT: RCC_STAT_ON or RCC_STAT_OFF
 * @return ErrorStatus_t OK if all good, ParameterError,ConfigurationError or NOK for other issues
 *
 *Notes: *you cannot turn OFF the system clock
 *		 *you cannot turn OFF the PLL source clock if PLL is system clock
 *
 */
 ...
 *
 *
 * @param SYS_ClOCK: SYS_CLK_clock to control
 * @return ErrorStatus_t OK if all good, ParameterError,ConfigurationError or NOK for other issues
 * Note: You cannot select a turned OFF clock as system clock
 *
 *
 */
 ...
 * @return ErrorStatus_t OK if all good, ParameterError,ConfigurationError or NOK for other issues
 * Notes: *Make sure PLL is turned off before using this function
 *		  *P values are P_2,P_4,P_6,P_8
 *		  *M range from 2<=M<=63
 *		  *N range from 192<=N<=432
 */

ErrorStatus_t RCC_ConfigurePLL(uint32 CLK_clock,uint32 P,uint16 M, uint16 N);

/**
 * @brief Enables peripheral
 *
 *
 * @param Bus: choose AHB1,APB1,APB2 or AHB2
 * @param BUSEN_peripheral: choose your desired peripheral to enable
 * @return ErrorStatus_t OK if all good, ParameterError,ConfigurationError or NOK for other issues
 * Note: *Make sure the periferal matches the bus selected
 *
 */
 ...
 *
 *
 * @param PPRE1_value: prescaler value for AHB1
 * @return ErrorStatus_t OK if all good, ParameterError,ConfigurationError or NOK for other issues
 *
 */
ErrorStatus_t RCC_SetAPB1Prescaler (uint64 PPRE_value);

/**
 * @brief sets APB2 prescaler value
 *
 *
 * @param PPRE2_value: prescaler value for AHB1
 * @return ErrorStatus_t OK if all good, ParameterError,ConfigurationError or NOK for other issues
 *
 */
ErrorStatus_t RCC_SetAPB2Prescaler (uint64 PPRE_value);
